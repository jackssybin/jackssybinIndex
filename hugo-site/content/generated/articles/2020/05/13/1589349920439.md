...
description: 1、查看当前登录用户信息 who命令： who缺省输出包括用户名、终端类型、登陆日期以及远程主机。 who /var/log/wtmp
  可以查看自从wtmp文件创建以来的每一次登陆情况 （1）b：查看系统最近一次启动时间 （2）H：打印每列的标题 users命令：
...
<div class="wrapper">
    ...
    <article class="post post--detail">
        <header>
            <h2><a rel="bookmark" href="/articles/2020/05/13/1589349920439.html">Linux - 查看用户登录记录</a></h2>
            <div class="meta">
                <span class="vditor-tooltipped vditor-tooltipped__n" aria-label="创建日期">
                    <i class="icon-date"></i>
                    <time>2020-05-13/2020-05-13</time>
                </span>
            </div>
        </header>
        <section class="article-guide">
      <div><strong>专题</strong><a href="/topics/linux-ops.html">Linux 运维与部署</a></div>
      <div><strong>导读</strong><span>本文归入「Linux 运维与部署」专题，主要记录：1、查看当前登录用户信息 who命令： who缺省输出包括用户名、终端类型、登陆日期以及远程主机。 who /var/log/wtmp 可以查看自从wtmp文件创建以来的每一次登陆情况 （1）b：查看系统最近一次启动时间 （2）H：打印每列的标题 users命令： 打印当前登录的用户，每个显示的用户名对应一个登陆会话。...</span></div>
  </section>
        
        <div id="more" class="vditor-reset post__content"><h2>1、查看当前登录用户信息</h2>
<p>who命令：
...
查看60天之前的用户登录历史</p>
<h2>5、ac命令</h2>
<p>根据/var/log/wtmp文件中的登陆和退出时间报告用户连接的时间（小时），默认输出报告总时间
（1）-p：显示每个用户的连接时间
（2）-d：显示每天的连接时间
（3）-y：显示年份，和-d配合使用</p>
</div>
...
<a class="tag" rel="tag" href="/tags/lastlog.html">lastlog</a>
      ...
      <a href="/articles/2020/05/11/1589167695782.html" rel="prev" class="fn-left vditor-tooltipped vditor-tooltipped__n" aria-label="Linux服务器kdevtmpfsi挖矿病毒解决方法">上一篇</a>
      <a href="/articles/2020/05/28/1590637980592.html" rel="next" class="fn-right vditor-tooltipped vditor-tooltipped__n" aria-label="MySQL如何利用索引优化ORDER BY排序">下一篇</a>
  </div>
        </footer>
