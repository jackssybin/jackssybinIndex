...
**这一行绿色直接回答了报告 8.4 第 2 条悬置了很久的问题：
`BAIDU_PUSH_TOKEN` 有效，站点归属验证有效，推送链路真正打通。**

推送 job 的关键日志：

```
Diff range: 3330e871e9b3bb9e431ad62117378ca3574eb089..HEAD
回退候选 4 条
去掉聚合页/去重后可推送: 4 / 4
推送批次 1/1 (4 条)
  结果: {"remain":4,"success":4}
  今日剩余配额: 4
✅ 累计成功推送 4 条
baidu-push exit code: 0
```
...
2. **配额只有个位数**（`remain=4`）。原来设的 `--max=50` 不现实：
   百度对超配额是**拒绝整批**而非接受前 N 条，撞上就整批白推。已下调到 10。
...
**顺手修掉一个验收脚本的 bug**：`post-deploy-verify.py` 里用 `r["head_sha"] == SHA` 做全等比较，
而 GitHub API 返回的是 40 位完整 sha，传 7 位短 sha 时**永远匹配不上**，
把一次已经成功的运行报成「10 分钟内未找到该 sha 的运行」，白等 5 分钟。
已改为前缀匹配（要求 `len(sha) >= 7`），本次重跑即通过。
...
4. **需要你留意的新情况**：百度给这个站的当日配额只有个位数。
   本次是改版后第一次成功推送，后续新文章当天基本能推完，
   但如果某天集中发多篇，会撞配额（届时 job 会显黄色 warning，不报错）。
   真要提升配额，得靠站点质量与持续更新，没有捷径。
