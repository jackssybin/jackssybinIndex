...
我在无任何密钥的情况下向 https://worldmonitor.app/mcp 发送 JSON-RPC tools/list 请求，实际返回 75 个工具，包括 get_country_risk、get_chokepoint_status、get_maritime_activity、get_conflict_events、get_market_data、get_sanctions_data、get_prediction_markets、get_world_brief 等，与 README「tools/list 公开、tools/call 需 Key 或 OAuth」的承诺一致。另有 REST API（OpenAPI 规范）、官方 CLI（npm 包 worldmonitor，别名 wm）与 Python/Go/Ruby 零依赖 SDK，并提供 llms.txt 与 agent-skills 发现文件。
