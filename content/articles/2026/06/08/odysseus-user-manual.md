...
> 这篇文章记录我从零开始使用 Odysseus 的完整过程。我会用第一人称梳理本地 AI 工作台搭建、模型配置、Chat、Agent、MCP、Deep Research、Cookbook、数据备份和安全访问，方便后来继续复盘，也方便刚接触 Odysseus 的朋友按步骤实践。
...
它不是大模型本身，而是一个统一入口。我需要给它配置模型来源，比如 Ollama、LM Studio、OpenAI、OpenRouter、vLLM 或 llama.cpp。
...
我的建议是不要一上来就研究所有功能。Odysseus 的能力很多，按阶段学习会更稳。
...
官方推荐 Docker，适合我在 Linux、服务器或希望服务隔离时使用：
...
如果是交互式终端，它会提示我输入账号和密码；如果是非交互环境，可能会生成临时密码并打印在终端或 Docker 日志里。
...
我会把 Chat 当成讨论区，把 Documents 当成正式产出区。
...
Cookbook 是我使用 Odysseus 跑本地模型时很重要的模块。
...
Windows 上如果我只是想用本地模型，最简单的方式通常是：
...
如果我是刚开始使用，我不会一上来就折腾 vLLM、CUDA、ROCm。先跑通 Ollama。

### Docker GPU

如果我用 Docker 并希望让容器访问 NVIDIA GPU，可以先诊断：
...
邮件功能涉及账号和授权码，我在配置时会比较谨慎，通常会先用测试邮箱。
...
如果我还可以进入 Settings，直接修改密码。

如果完全无法登录，我需要通过 `data/auth.json` 或项目的认证管理逻辑重置。我不会直接乱改，而是先备份：
...
当我需要让 Odysseus 操作外部工具时，再接入 MCP。
...
我不会急着一次性学完。每天掌握一个模块，一两周后就能把 Odysseus 用成自己的 AI 工作台。
